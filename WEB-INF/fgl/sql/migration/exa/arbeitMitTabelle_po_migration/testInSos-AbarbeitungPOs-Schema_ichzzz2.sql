...
--Merke:
--- IDEE: Die PO ohne Vertiefung zuletzt. Dann sind auch alle Studis mit Vertiefung angelegt, deren Leistungen
---       und dies rundet das dann ab.
---ALTERNATIVE IDEE: Nach einer Vertiefung diese noch einmal einbauen...
...
update ichzzz.po_migration
SET migriert = true
WHERE reihenfolge <=19;


--- REIHENFOLGE GGFS. ÄNDERN, durch Dreieckstauschtausch möglich wenn man keine Lücken in der Reihenfolge haben will.
update ichzzz.po_migration
SET reihenfolge =23
WHERE reihenfolge =21 ;

update ichzzz.po_migration
SET reihenfolge =21
WHERE reihenfolge =22 ;

update ichzzz.po_migration
SET reihenfolge =22
WHERE reihenfolge =23 ;

--------------------------------------------------------------
---------------------------------------------------------------
--WERTE ÄNDERN
update ichzzz.po_migration
set tubaf_po = '82-049---2014'
WHERE reihenfolge = 1;
